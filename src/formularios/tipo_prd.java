 ...
 * @author mi
 */
public class tipo_prd extends javax.swing.JFrame {
 ...
 //javax.swing.table.DefaultTableModel cursor;
  private int posreg;
    ...
    private void txtdescripcionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtdescripcionActionPerformed
        ...
        ValidarDuplicados = "SELECT ti_pro_des FROM tipo_produc "
                        + "WHERE trim(ti_pro_des) ='"+TextoValidar+"' ";
        try
        {
  ...
  private void des_botones() //metodo para deshabilitar botones
	...
	// bborrar.setEnabled(true);
	 ...
	 bcancelar.setEnabled(false);
       ...
       private void boton_borrar()	// metodo para hab botones
     {
         bagregar.setEnabled(true);
	// bborrar.setEnabled(true);
         beditar.setEnabled(true);
     ...
     private void gencod()
    {
        try {
            //       Main.inicio = "no";
                   ver_conex conn = new ver_conex();
                   conn.sentencia = conn.conexion.createStatement();
                   conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(tip_prd_cod),0)+ 1 AS xxx FROM tipo_producto");
                   conn.resultado.next();
       ...
       private void grabar()
    {
        try {
            ...
            if(bandera.equals("agregar"))
             {
              ...
              conn.sentencia.executeUpdate("INSERT INTO tipo_producto(tip_prd_cod,tip_prd_dec,tip_prd_est)values("+txtcod.getText()+",'"+txtdescripcion.getText()+"','"+estado+"')");
             cargar_grilla();
            ...
            if(bandera.equals("editar"))
             {
               conn.sentencia.executeUpdate("UPDATE tipo_producto SET tip_prd_dec ='"+txtdescripcion.getText()+"' WHERE tip_prd_cod ="+txtcod.getText());
            cargar_grilla();
             }
...
this.bcancelar.doClick();
        ...
        private void buscar()
    {
       ...
       ("SELECT * FROM tipo_producto WHERE tip_prd_cod = "+txtcod.getText());
            boolean encontro = conn.resultado.next();

            if(encontro == true)
            {
                txtdescripcion.setText(conn.resultado.getString("tip_prd_dec"));
   ...
   if(bandera.equals("borrar"))
                {
                    int mensaje = JOptionPane.showConfirmDialog(this,"Deseas Borrar-->"+txtdescripcion.getText(),"Confirmar",JOptionPane.YES_NO_OPTION);
                     ...
                     {
                          conn.sentencia = conn.conexion.createStatement();
                          conn.sentencia.executeUpdate("DELETE FROM TIPO_PRODUCTO Where tip_prd_cod= "+txtcod.getText()+"");
  ...
  private void cargar_grilla()
  {
      try
      {
           for (int i = grilla.getRowCount()-1; i>=0; i--)
            ...
            ver_conex conn = new ver_conex();
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT * from tipo_producto");
           
            while(conn.resultado.next())
            {
                Object[] datos = {conn.resultado.getString("tip_prd_cod"),conn.resultado.getString("tip_prd_dec"), conn.resultado.getString("tip_prd_est"),
   ...
   public void eligio()
     ...
     private void comparar()
     {
            ...
            texto.toUpperCase();
            ValidarDuplicados="SELECT ti_pro_des as descripcion FROM tipo_produc WHERE ti_pro_des='"+texto+"'";
            try
            {
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery(ValidarDuplicados);
