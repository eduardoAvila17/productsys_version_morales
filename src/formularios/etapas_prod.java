 ...
 * @author mi
 */
public class etapas_prod extends javax.swing.JFrame {
...
if(txtdescripcion.getText().trim().length()==0){
        ...
        ValidarDuplicados = "SELECT etap_desc FROM control_etapas "
                        + "WHERE trim(etap_desc) ='"+TextoValidar+"' ";
        try
        {
     ...
     private void gencod()
    {
        try {
            //       Main.inicio = "no";
                   ver_conex conn = new ver_conex();
                   conn.sentencia = conn.conexion.createStatement();
                   conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(etap_cod),0)+ 1 AS xxx Fcontrol_etapasROM ");
                   conn.resultado.next();
       ...
       private void grabar()
    {
        try {
            ...
            if(bandera.equals("agregar"))
             {
              ...
              conn.sentencia.executeUpdate("INSERT INTO control_etapas(etap_cod,etap_desc) values("+txtcod.getText()+",'"+txtdescripcion.getText()+"')");
             cargar_grilla();
            ...
            if(bandera.equals("editar"))
             {
               conn.sentencia.executeUpdate("UPDATE control_etapas SET etap_desc ='"+txtdescripcion.getText()+"' WHERE etap_cod ="+txtcod.getText());
            cargar_grilla();
             }
...
this.bcancelar.doClick();
        ...
        private void buscar()
    {
       ...
       ("SELECT * FROM control_etapas WHERE etap_cod = "+txtcod.getText());
            boolean encontro = conn.resultado.next();

            if(encontro == true)
            {
                txtdescripcion.setText(conn.resultado.getString("etap_desc"));
   ...
   if(bandera.equals("borrar"))
                {
                    int mensaje = JOptionPane.showConfirmDialog(this,"Deseas Borrar-->"+txtdescripcion.getText(),"Confirmar",JOptionPane.YES_NO_OPTION);
                     ...
                     {
                          conn.sentencia = conn.conexion.createStatement();
                          conn.sentencia.executeUpdate("delete from  control_etapas where  etap_cod = "+txtcod.getText());
  ...
  private void cargar_grilla()
  {
      try
      {
           for (int i = grilla.getRowCount()-1; i>=0; i--)
            ...
            ver_conex conn = new ver_conex();
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT * from control_etapas");
           
            while(conn.resultado.next())
            {
                Object[] datos = {conn.resultado.getString("etap_cod"),conn.resultado.getString("etap_desc"),
    ...
    private void comparar()
     {
            ...
            texto.toUpperCase();
            ValidarDuplicados="SELECT etap_desc as descripcion FROM control_etapas WHERE etap_desc='"+texto+"'";
            try
            {
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery(ValidarDuplicados);
