 ...
 //javax.swing.table.DefaultTableModel cursor;
  private int posreg;
    ...
    private void txtdescripcionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtdescripcionActionPerformed
        ...
        ValidarDuplicados = "SELECT presen_des FROM presentacion "
                        + "WHERE trim(presen_des) ='"+TextoValidar+"' ";
        try
        {
     ...
     private void gencod()
    {
        try {
            //       Main.inicio = "no";
                   ver_conex conn = new ver_conex();
                   conn.sentencia = conn.conexion.createStatement();
                   conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(presen_cod),0)+ 1 AS xxx FROM presentacion");
                   conn.resultado.next();
       ...
       private void grabar()
    {
        try {
            ...
            if(bandera.equals("agregar"))
             {
              ...
              conn.sentencia.executeUpdate("INSERT INTO presentacion(presen_cod,presen_des,prese_est)values("+txtcod.getText()+",'"+txtdescripcion.getText()+"','ACTIVO')");
             cargar_grilla();
            ...
            if(bandera.equals("editar"))
             {
               conn.sentencia.executeUpdate("UPDATE presentacion SET presen_des ='"+txtdescripcion.getText()+"' WHERE presen_cod ="+txtcod.getText());
            cargar_grilla();
             }
...
this.bcancelar.doClick();
        ...
        private void buscar()
    {
       ...
       ("SELECT * FROM presentacion WHERE presen_cod = "+txtcod.getText());
            boolean encontro = conn.resultado.next();

            if(encontro == true)
            {
                txtdescripcion.setText(conn.resultado.getString("presen_des"));
   ...
   if(bandera.equals("borrar"))
                {
                    int mensaje = JOptionPane.showConfirmDialog(this,"Deseas Borrar-->"+txtdescripcion.getText(),"Confirmar",JOptionPane.YES_NO_OPTION);
                     ...
                     {
                          conn.sentencia = conn.conexion.createStatement();
                          conn.sentencia.executeUpdate("UPDATE presentacion SET prese_est ='Borrado' Where presen_cod= "+txtcod.getText()+"");
  ...
  private void cargar_grilla()
  {
      try
      {
           for (int i = grilla.getRowCount()-1; i>=0; i--)
            ...
            ver_conex conn = new ver_conex();
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT * from presentacion");
           
            while(conn.resultado.next())
            {
                Object[] datos = {conn.resultado.getString("presen_cod"),conn.resultado.getString("presen_des"),conn.resultado.getString("prese_est"),
    ...
    private void comparar()
     {
            ...
            texto.toUpperCase();
            ValidarDuplicados="SELECT presen_des as descripcion FROM presentacion WHERE presen_des='"+texto+"'";
            try
            {
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery(ValidarDuplicados);
