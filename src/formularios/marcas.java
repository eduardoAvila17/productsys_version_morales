...
if(txtdescripcion.getText().trim().length()==0){
        ...
        ValidarDuplicados = "SELECT mar_desc FROM marcas "
                        + "WHERE trim(mar_desc) ='"+TextoValidar+"' ";
        try
        {
     ...
     private void gencod()
    {
        try {
            //       Main.inicio = "no";
                   ver_conex conn = new ver_conex();
                   conn.sentencia = conn.conexion.createStatement();
                   conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(mar_cod),0)+ 1 AS xxx FROM marcas");
                   conn.resultado.next();
        ...
        try {
            ...
            if(bandera.equals("agregar"))
             {
              ...
              conn.sentencia.executeUpdate("INSERT INTO marcas(mar_cod,mar_desc,mar_est) values("+txtcod.getText()+",'"+txtdescripcion.getText()+"','"+estado+"')");
             cargar_grilla();
          //   this.Bborrar.doClick();
            ...
            if(bandera.equals("editar"))
             {
               conn.sentencia.executeUpdate("UPDATE marcas SET mar_desc ='"+txtdescripcion.getText()+"' WHERE mar_cod ="+txtcod.getText());
            cargar_grilla();
             }
...
//this.Bborrar.doClick();
        ...
        private void buscar()
    {
       ...
       ("SELECT * FROM marcas WHERE mar_cod = "+txtcod.getText());
            boolean encontro = conn.resultado.next();

            if(encontro == true)
            {
                txtdescripcion.setText(conn.resultado.getString("mar_desc"));
   ...
   if(bandera.equals("borrar"))
                {
                    int mensaje = JOptionPane.showConfirmDialog(this,"Deseas Borrar-->"+txtdescripcion.getText(),"Confirmar",JOptionPane.YES_NO_OPTION);
                     if(mensaje == JOptionPane.YES_OPTION)//si quiere borrar
                     {
                         // conn.sentencia = conn.conexion.createStatement();
                           conn.sentencia = conn.conexion.createStatement();
                           conn.sentencia.executeUpdate("delete from marcas Where mar_cod= "+txtcod.getText()+"");
  ...
  private void cargar_grilla()
  {
      try
      {
           for (int i = grilla.getRowCount()-1; i>=0; i--)
            ...
            ver_conex conn = new ver_conex();
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT * from marcas");
           
            while(conn.resultado.next())
            {
                Object[] datos = {conn.resultado.getString("mar_cod"),conn.resultado.getString("mar_desc"),conn.resultado.getString("mar_est"),
   ...
   private void comparar()
     {
            ...
            texto.toUpperCase();
            ValidarDuplicados="SELECT mar_desc as descripcion FROM marcas WHERE mar_desc='"+texto+"'";
            try
            {
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery(ValidarDuplicados);
